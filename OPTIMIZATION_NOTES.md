...
### 4. Command batching/debouncing
- `up/down` brightness changes are debounced and coalesced into a single PUT per burst.
- The debounce window is configurable via `command_debounce_ms` (default 500ms).
...
| Rapid sequence | `N` commands → `2N` requests | `N` commands → `~1` request per burst (after first sync) |
